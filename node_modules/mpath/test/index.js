  ...
  describe('get', function(){
    ...
    describe('without `special`', function(){
      it('works', function(done){
        ...
        assert.deepEqual({ name: 'one' }, mpath.get('reviews.0', o));
        assert.deepEqual('one', mpath.get('reviews.0.name', o));
        assert.deepEqual('two', mpath.get('reviews.1.name', o));
        assert.deepEqual('three', mpath.get('reviews.2.name', o));

        assert.deepEqual([{},{ reviews: [{val: 'twoo'}]}]
            , mpath.get('reviews.2.reviews', o));

        assert.deepEqual({ reviews: [{val: 'twoo'}]}
            , mpath.get('reviews.2.reviews.1', o));

        assert.deepEqual('twoo', mpath.get('reviews.2.reviews.1.reviews.0.val', o));
      ...
      it('handles array.array notation', function(done){
        assert.deepEqual(
            [undefined, undefined, [{}, {reviews:[{val:'twoo'}]}]]
          , mpath.get('reviews.reviews', o)
        );
        done();
      })
    ...
    describe('with `special`', function(){
      describe('that is a string', function(){
        it('works', function(done){
          ...
          assert.deepEqual([
            ...
            , { name: 'three'
              , reviews: [{},{ reviews: [{val: 'twoo'}]}]
              , _doc: { name: '3', reviews: [{},{ _doc: { reviews: [{ val: 2 }] }}]}}],
            mpath.get('reviews', o, special));

          assert.deepEqual({ name: 'one' }, mpath.get('reviews.0', o, special));
          assert.deepEqual('one', mpath.get('reviews.0.name', o, special));
          assert.deepEqual('2', mpath.get('reviews.1.name', o, special));
          assert.deepEqual('3', mpath.get('reviews.2.name', o, special));
          assert.deepEqual('nice', mpath.get('reviews.2.name', o, special, function (v) {
            return '3' === v ? 'nice' : v;
          }));

          assert.deepEqual([{},{ _doc: { reviews: [{ val: 2 }] }}]
              , mpath.get('reviews.2.reviews', o, special));

          assert.deepEqual({ _doc: { reviews: [{val: 2}]}}
              , mpath.get('reviews.2.reviews.1', o, special));

          assert.deepEqual(2, mpath.get('reviews.2.reviews.1.reviews.0.val', o, special));
          done();
        })
        ...
        it('handles array.property dot-notation', function(done){
          assert.deepEqual(
            ['one', '2', '3']
            , mpath.get('reviews.name', o, special)
          );
          assert.deepEqual(
            ['one', 2, '3']
            , mpath.get('reviews.name', o, special, function (v) {
              return '2' === v ? 2 : v
            })
          );
          done();
        ...
        it('handles array.array notation', function(done){
          assert.deepEqual(
              [undefined, undefined, [{}, {_doc: { reviews:[{val:2}]}}]]
            , mpath.get('reviews.reviews', o, special)
          );
          done();
        })
        ...
        it('handles array.array.index.array', function(done){
          assert.deepEqual(
            ...
            , mpath.get('reviews.reviews.1.reviews', o, special)
          );
          done();
        })
        ...
        it('handles array.array.index.array.prop', function(done){
          assert.deepEqual(
            ...
            , mpath.get('reviews.reviews.1.reviews.val', o, special)
          );
          assert.deepEqual(
              ['nil', 'nil', [2]]
            , mpath.get('reviews.reviews.1.reviews.val', o, special, function (v) {
      ...
      describe('that is a function', function(){
        ...
        it('works', function(done){
          ...
          assert.deepEqual({ name: 'one' }, mpath.get('reviews.0', o, special));
          assert.deepEqual('one', mpath.get('reviews.0.name', o, special));
          assert.deepEqual('two', mpath.get('reviews.1.name', o, special));
          assert.deepEqual('three', mpath.get('reviews.2.name', o, special));
          assert.deepEqual('nice', mpath.get('reviews.2.name', o, special, function (v) {
          ...
          assert.deepEqual('twoo', mpath.get('reviews.2.reviews.1.reviews.0.val', o, special));

          var overide = false;
          assert.deepEqual('twoo', mpath.get('reviews.8.reviews.1.reviews.0.val', o, function (obj, path) {
  ...
  describe('set', function(){
    describe('without `special`', function(){
      ...
      it('works', function(done){
        ...
        mpath.set('reviews.1.name', 'ttwwoo', o);
        assert.deepEqual({ name: 'ttwwoo', _doc: { name: '2' }}, o.reviews[1]);

        mpath.set('reviews.2.reviews.1.reviews.0.expand', 'added', o);
        assert.deepEqual(
            { val: 'twoo', expand: 'added'}
          , o.reviews[2].reviews[1].reviews[0]);

        mpath.set('reviews.2.reviews.1.reviews.2', 'added', o);
        assert.equal(3, o.reviews[2].reviews[1].reviews.length);
        assert.deepEqual(
            { val: 'twoo', expand: 'added'}
          , o.reviews[2].reviews[1].reviews[0]);
        assert.deepEqual(
            undefined
          , o.reviews[2].reviews[1].reviews[1]);
        assert.deepEqual(
            'added'
          , o.reviews[2].reviews[1].reviews[2]);
    ...
    describe('with `special`', function(){
      ...
      it('works', function(done){
        ...
        mpath.set('reviews.1.name', 'ttwwoo', o, special);
        assert.deepEqual({ name: 'two', _doc: { name: 'ttwwoo' }}, o.reviews[1]);

        mpath.set('reviews.2.reviews.1.reviews.0.expander', 'adder', o, special, function (v) {
          return 'super'
        });
        assert.deepEqual(
            { val: 2, expander: 'super'}
          , o.reviews[2]._doc.reviews[1]._doc.reviews[0]);

        mpath.set('reviews.2.reviews.1.reviews.0.expander', 'adder', o, special);
        assert.deepEqual(
            { val: 2, expander: 'adder'}
          , o.reviews[2]._doc.reviews[1]._doc.reviews[0]);

        mpath.set('reviews.2.reviews.1.reviews.2', 'set', o, special);
        assert.equal(3, o.reviews[2]._doc.reviews[1]._doc.reviews.length);
        assert.deepEqual(
            { val: 2, expander: 'adder'}
          , o.reviews[2]._doc.reviews[1]._doc.reviews[0]);
        assert.deepEqual(
            undefined
          , o.reviews[2]._doc.reviews[1]._doc.reviews[1]);
        assert.deepEqual(
            'set'
          , o.reviews[2]._doc.reviews[1]._doc.reviews[2]);
        done();
      })
    ...
    describe('get/set integration', function(){
      ...
      it('array.prop', function(done){
        mpath.set('reviews.name', ['this', 'was', 'changed'], o);
        ...
        mpath.set('reviews.name', ['also', 'changed', 'this'], o, special);
        ...
        assert.deepEqual([
          ...
          , { name: 'changed'
              , reviews: [{},{ reviews: [{val: 'twoo'}]}]
              , _doc: { name: 'this', reviews: [{},{ _doc: { reviews: [{ val: 2 }] }}]  }}
        ], o.reviews);
