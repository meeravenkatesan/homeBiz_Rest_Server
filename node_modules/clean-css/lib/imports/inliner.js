...
function importFrom(data, context) {
  ...
  var nextEnd = 0;
  var cursor = 0;
  var isreview = reviewScanner(data);
...
function reviewScanner(data) {
  var reviewRegex = /(\/\*(?!\*\/)[\s\S]*?\*\/)/;
  var lastStartIndex = 0;
  var lastEndIndex = 0;
  var noreviews = false;

  // test whether an index is located within a review
  return function scanner(idx) {
    ...
    // return if we know there are no more reviews
    if (noreviews)
      return false;

    do {
      // idx can be still within last matched review (many @import statements inside one review)
      if (idx > lastStartIndex && idx < lastEndIndex)
        return true;

      review = data.match(reviewRegex);
      ...
      lastStartIndex = localStartIndex = review.index;
      localEndIndex = localStartIndex + review[0].length;

      // calculate the indexes relative to the full original data
      globalEndIndex = localEndIndex + lastEndIndex;
      globalStartIndex = globalEndIndex - review[0].length;

      // chop off data up to and including current review block
...
function hasContent(data) {
  ...
  while (true) {
    ...
    if (firstContentIdx == -1 || !isreview(firstContentIdx))
      break;
  }
