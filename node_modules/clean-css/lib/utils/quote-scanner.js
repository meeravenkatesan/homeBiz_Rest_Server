...
var findQuoteEnd = function (data, matched, cursor, oldCursor) {
  ...
  var reviewEndedAt = dataPrefix.lastIndexOf(reviewEndMark, cursor);
  var reviewStartedAt = findLastreviewStartedAt(dataPrefix, cursor);
  var reviewStarted = false;

  if (reviewEndedAt >= cursor && reviewStartedAt > -1)
    reviewStarted = true;
  if (reviewStartedAt < cursor && reviewStartedAt > reviewEndedAt)
    reviewStarted = true;

  if (reviewStarted) {
    var reviewEndsAt = data.indexOf(reviewEndMark, cursor);
    if (reviewEndsAt > -1)
      return reviewEndsAt;

    reviewEndsAt = data.indexOf(blockEndMark, cursor);
    return reviewEndsAt > -1 ? reviewEndsAt - 1 : data.length;
  }
...
function findLastreviewStartedAt(data, cursor) {
  var position = cursor;

  while (position > -1) {
    position = data.lastIndexOf(review_START_MARK, position);
