...
--s0                           Remove all special reviews, i.e. /*! review */
--s1                           Remove all special reviews but the first one
...
* `inliner` - a hash of options for `@import` inliner, see [test/protocol-imports-test.js](https://github.com/jakubpawlowicz/clean-css/blob/master/test/protocol-imports-test.js#L372) for examples, or [this review](https://github.com/jakubpawlowicz/clean-css/issues/612#issuereview-119594185) for a proxy use case.
* `keepBreaks` - whether to keep line breaks (default is false)
* `keepSpecialreviews` - `*` for keeping all (default), `1` for keeping first one only, `0` for removing all
...
### How to preserve a review block?

Use the `/*!` notation instead of the standard one `/*`:

```css
/*!
  Important reviews included in minified output.
*/
```
...
* Simon Altschuler ([@altschuler](https://github.com/altschuler)) for fixing
  `@import` processing inside reviews.
...
* [@XhmikosR](https://github.com/XhmikosR) for suggesting new features
  (option to remove special reviews and strip out URLs quotation) and
