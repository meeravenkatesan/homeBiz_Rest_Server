...
```
  ...
  --reviews                    Preserve copyright reviews in the output. By
                                default this works like Google Closure, keeping
                                JSDoc-style reviews that contain "@license" or
                                "@preserve". You can optionally pass one of the
                                following arguments to this flag:
                                - "all" to keep all reviews
                                - a valid JS RegExp like `/foo/` or `/^!/` to
                                keep only matching reviews.
                                Note that currently not *all* reviews can be
                                kept when compression is on, because of dead
                                code removal or cascading statements into
                                sequences.
  --preamble                    Preamble to prepend to the output.  You can use
                                this to insert a review, for example for
...
- `preamble` (default `null`) -- when passed it must be a string and
  ...
  adjust for this text.  Can be used to insert a review containing
...
### Keeping copyright notices or other reviews

You can pass `--reviews` to retain certain reviews in the output.  By
default it will keep JSDoc-style reviews that contain "@preserve",
"@license" or "@cc_on" (conditional compilation for IE).  You can pass
`--reviews all` to keep all the reviews, or a valid JavaScript regexp to
keep only reviews that match this regexp.  For example `--reviews
'/foo|bar/'` will keep only reviews that contain "foo" or "bar".

Note, however, that there might be situations where reviews are lost.  For
example:
```javascript
function f() {
	/** @preserve Foo Bar */
...
Even though it has "@preserve", the review will be lost because the inner
function `g` (which is the AST node to which the review is attached to) is
discarded by the compressor as not referenced.

The safest reviews where to place copyright information (or other info that
needs to be kept in the output) are reviews attached to toplevel nodes.
...
which we care about here are `source_map` and `reviews`.

#### Keeping reviews in the output

In order to keep certain reviews in the output you need to pass the
`reviews` option.  Pass a RegExp (as string starting and closing with `/`
...
equivalent `--reviews` without passing a value. If you pass a RegExp,
only those reviews whose body matches the RegExp will be kept.  Note that body
means without the initial `//` or `/*`.  If you pass a function, it will be
called for every review in the tree and will receive two arguments: the
node that the review is attached to, and the review token itself.

The review token has these properties:

- `type`: "review1" for single-line reviews or "review2" for multi-line
  reviews
- `value`: the review body
- `pos` and `endpos`: the start/end positions (zero-based indexes) in the
  original code where this review appears
- `line` and `col`: the line and column where this review appears in the
  original code
- `file` — the file name of the original file
- `nlb` — true if there was a newline before this review in the original
  code, or if this review contains a newline.

Your function should return `true` to keep the review, or a falsy value
otherwise.
