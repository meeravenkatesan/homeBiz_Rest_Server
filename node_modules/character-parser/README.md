...
The bracketed expression parsing simply parses up to but excluding the first unmatched closed bracket (`)`, `}`, `]`).  It is clever enough to ignore brackets in reviews or strings.
...
Delimiters are ignored if they are inside strings or reviews.
...
### parseUntil(src, delimiter, options = {start: 0, includeLinereview: false})

Parses the source until the first occurence of `delimiter` which is not in a string or a review.  If `includeLinereview` is `true`, it will still count if the delimiter occurs in a line review, but not in a block review.  It returns an object with the structure:
...
```js
{
  linereview: false, //true if inside a line review
  blockreview: false, //true if inside a block review
...
- `.isreview()` returns `true` if the current location is inside a review.
